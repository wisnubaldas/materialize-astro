...
import customizerMarkup from '/src/assets/js/_template-customizer/_template-customizer.html';
...
const getAttr = (el, attr, fallback = '') =>
  el && typeof el.getAttribute === 'function' ? el.getAttribute(attr) ?? fallback : fallback;
...
const rootStyles =
  typeof window !== 'undefined' && typeof window.getComputedStyle === 'function' && DOC_EL
...
class TemplateCustomizer {
  ...
  _loadSettings() {
    ...
    if (!this._hasControls('rtl'))
      this.settings.rtl = document.documentElement.getAttribute('dir') === 'rtl';
  ...
  _setup(_container = document) {
    ...
    if (
      !this._hasControls('contentLayout headerType layoutCollapsed layoutNavbarOptions rtl', true)
    ) {
