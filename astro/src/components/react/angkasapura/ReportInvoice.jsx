...
export default function ReportInvoice() {
  ...
  const [monthlyChartComponent, setMonthlyChartComponent] = useState(null);
  const [monthlyChartLibraryError, setMonthlyChartLibraryError] = useState('');
  ...
  useEffect(() => {
    ...
    const loadChartLibrary = async () => {
      try {
        const [{ Line }, chartJs] = await Promise.all([import('react-chartjs-2'), import('chart.js')]);
        ...
        if (isMounted) {
          setMonthlyChartComponent(() => Line);
        }
      } catch (error) {
        if (isMounted) {
          setMonthlyChartLibraryError('Gagal memuat library chart invoice.');
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchMonthlyReport = async () => {
      ...
      try {
        ...
        if (isMounted) {
          const message = resolveErrorMessage(error, 'Gagal mengambil data grafik invoice bulanan.');
  ...
  useEffect(() => {
    ...
    const fetchStatusSummary = async () => {
      ...
      try {
        ...
        if (isMounted) {
          const message = resolveErrorMessage(error, 'Gagal mengambil ringkasan status invoice.');
  ...
  const monthlyChartOptions = useMemo(
    () => ({
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (context) => `${context.parsed.y ?? 0} invoice`,
          },
        },
      },
      elements: {
  ...
  const monthlyChartData = useMemo(
    () => ({
      ...
      datasets: [
        {
          label: `Invoice Terkirim ${selectedYear}`,
  ...
  const MonthlyChartComponent = monthlyChartComponent;
          ...
          {errorMonthly ? (
            ...
            <div className="py-5 text-center text-muted">
              <div
                ...
                aria-hidden="true"
              ></div>
              <span>Memuat library grafik...</span>
            </div>
          ) : (
            <>
              <div style={{ minHeight: '300px' }}>
                {MonthlyChartComponent ? (
                  <MonthlyChartComponent options={monthlyChartOptions} data={monthlyChartData} />
                ) : null}
              </div>
              {!hasMonthlyData ? (
