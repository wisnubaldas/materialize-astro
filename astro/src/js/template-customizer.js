...
const DOC_EL = DOC_AVAILABLE ? document.documentElement : null;
const FALLBACK_CLASSLIST = {
  contains: () => false,
  add: () => {},
  remove: () => {},
};
const cl = DOC_EL ? DOC_EL.classList : FALLBACK_CLASSLIST;
const HTML_EL =
  DOC && typeof DOC.getElementsByTagName === "function"
...
const DEFAULT_THEME = getAttr(DOC_EL, "data-bs-theme", "light") || "light";
const DEFAULT_SKIN = getAttr(HTML_EL, "data-skin", 0) || 0;
const DEFAULT_CONTENT_LAYOUT = cl.contains("layout-wide") ? "wide" : "compact";

let headerType = "static";
...
const DEFAULT_TEXT_DIR = getAttr(DOC_EL, "dir") === "rtl";
...
const rootStyles =
  ...
  DOC_EL
    ? window.getComputedStyle(DOC_EL)
    : null;
...
class TemplateCustomizer {
  constructor({
    ...
    this.settings.defaultNavbarType =
      typeof defaultNavbarType !== "undefined"
        ? defaultNavbarType
    ...
    this.settings.availableColors =
      availableColors || TemplateCustomizer.COLORS;
    ...
    this.settings.availableContentLayouts =
      availableContentLayouts || TemplateCustomizer.CONTENT;
    this.settings.availableHeaderTypes =
      availableHeaderTypes || TemplateCustomizer.HEADER_TYPES;
    this.settings.availableMenuCollapsed =
      availableMenuCollapsed || TemplateCustomizer.LAYOUTS;
    this.settings.availableNavbarOptions =
      availableNavbarOptions || TemplateCustomizer.NAVBAR_OPTIONS;
    this.settings.availableDirections =
      availableDirections || TemplateCustomizer.DIRECTIONS;
    this.settings.defaultSkin = this._getDefaultSkin(
      typeof defaultSkin !== "undefined" ? defaultSkin : DEFAULT_SKIN
    );
  ...
  setHeaderType(pos, updateStorage = true) {
    ...
    if (
      !["static", "static-offcanvas", "fixed", "fixed-offcanvas"].includes(pos)
    )
      return;
  ...
  setDropdownOnHover(open, updateStorage = true) {
    ...
    globalConfig.showDropdownOnHover = open;
    ...
    window.Helpers.mainMenu = new Menu(menuElement, {
      orientation: "horizontal",
      closeChildren: true,
      showDropdownOnHover: globalConfig.showDropdownOnHover,
    });
  ...
  _loadSettings() {
    ...
    this.settings.rtl =
      rtlOption !== "" ? rtlOption === "true" : this.settings.defaultTextDir;
    ...
    // Style
    this.settings.themesOpt = this.settings.themes.includes(theme)
      ? theme
    ...
    // Semi Dark
    this.settings.semiDark = semiDark
      ? semiDark === "true"
      : this.settings.defaultSemiDark;
    ...
    this.settings.contentLayout =
      contentLayout || this.settings.defaultContentLayout;

    // Layout Collapsed
    this.settings.layoutCollapsed = collapsedMenu
      ? collapsedMenu === "true"
      : this.settings.defaultMenuCollapsed;
    // Add layout-menu-collapsed class to the body if the menu is collapsed
    if (this.settings.layoutCollapsed)
    ...
    // Navbar Option
    this.settings.layoutNavbarOptions = ["static", "sticky", "hidden"].includes(
      navbarOption
    )
      ? navbarOption
      : this.settings.defaultNavbarType;
    ...
    // Footer Fixed
    this.settings.layoutFooterFixed = fixedFooter
      ? fixedFooter === "true"
      : this.settings.defaultFooterFixed;
  ...
  _setup(_container = document) {
      ...
      if (isIcon) {
        ...
        fetch(`${assetsPath}img/customizer/${image}`)
          ...
          .then((svgContent) => {
            ...
            divElement.querySelector(".custom-option-body").innerHTML =
              svgContent;
          })
    ...
    this.container.setAttribute(
      "style",
      `visibility: ${this.settings.displayCustomizer ? "visible" : "hidden"}`
    );
    ...
    const colorOpt = colorW.querySelector(
    ...
    if (!this._hasControls("color")) {
      ...
      const colorSelector = colorOpt.querySelector(
    ...
    const themeOpt = themeW.querySelector(
    ...
    if (!this._hasControls("theme")) {
      ...
      this.settings.availableThemes.forEach((theme) => {
        const themeEl = createOptionElement(
          theme.name,
          theme.title,
          "customRadioIcon",
          theme.image,
      ...
      if (themeOpt.querySelector(`input[value="${this.settings.themesOpt}"]`)) {
        themeOpt
          .querySelector(`input[value="${this.settings.themesOpt}"]`)
          .setAttribute("checked", "checked");
      }
      ...
      const themeCb = (e) => {
        ...
        if (this._hasControls("semiDark")) {
          const semiDarkL = this.container.querySelector(
    ...
    const skinsWInner = skinsW.querySelector(
    ...
    if (!this._hasControls("skins")) {
      ...
      this.settings.availableSkins.forEach((skin) => {
        const skinEl = createOptionElement(
          skin.name,
          skin.title,
          "skinRadios",
          skin.image
      ...
      skinsWInner
        .querySelector(`input[value="${this.settings.skin.name}"]`)
        .setAttribute("checked", "checked");
      document.documentElement.setAttribute(
        "data-skin",
    ...
    const semiDarkSwitch = this.container.querySelector(
      ".template-customizer-semi-dark-switch"
    );
    const semiDarkSection = this.container.querySelector(
      ".template-customizer-semiDark"
    );
    ...
    if (
      ...
      const layoutCollapsedW = this.container.querySelector(
      ...
      if (!this._hasControls("layoutCollapsed")) {
        ...
        const layoutCollapsedOpt = layoutCollapsedW.querySelector(
      ...
      if (!this._hasControls("contentLayout")) {
        ...
        const contentOpt = contentWrapper.querySelector(
      ...
      // Header Layout Type
      const headerTypeW = this.container.querySelector(
        ".template-customizer-headerOptions"
      );
      const templateName = document.documentElement
        .getAttribute("data-template")
        .split("-");
      if (!this._hasControls("headerType")) {
        headerTypeW.parentNode.removeChild(headerTypeW);
      } else {
        const headerOpt = headerTypeW.querySelector(
      ...
      const navbarOption = this.container.querySelector(
      ...
      if (!this._hasControls("layoutNavbarOptions")) {
        ...
        const navbarTypeOpt = navbarOption.querySelector(
      ...
      // RTL
      const directionW = this.container.querySelector(
      ...
      if (!this._hasControls("rtl")) {
        ...
        const rtlCb = (e) => {
          ...
          if (languageDropdown) {
            const dropdownItem = languageDropdown.querySelector(
    ...
    setTimeout(() => {
      const layoutCustom = this.container.querySelector(
        ".template-customizer-layout"
      );
      const layoutTheme = this.container.querySelector(
        ".template-customizer-theming"
      );

      const checkSemiDarkWrapper =
        document.documentElement.getAttribute("data-bs-theme");
      let checkSemiDark = false;

      if (
        checkSemiDarkWrapper === "light" &&
        ...
        if (checkSemiDark === true) {
          const semiDarkSwitch = layoutTheme.querySelector(
            ".template-customizer-semi-dark-switch"
          );
          semiDarkSwitch.setAttribute("checked", "checked");
        }
  ...
  _loadingState(enable, skins) {
    this.container.classList[enable ? "add" : "remove"](
      `template-customizer-loading${skins ? "-theme" : ""}`
    );
  }
  ...
  _getElementFromString(str) {
    ...
    const element = wrapper.firstElementChild || wrapper.firstChild;

    if (!element || element.nodeType !== 1) {
      throw new Error("Invalid template customizer markup.");
    }
  ...
  _showResetBtnNotification(show = true) {
    setTimeout(() => {
      const resetBtnAttr = this.container.querySelector(
  ...
  _getDefaultSkin(skinId) {
    const skin =
      typeof skinId === "string"
        ? this._getSkinByName(skinId, false)
        : this.settings.availableSkins[skinId];
    if (!skin) {
...
if (typeof window !== "undefined") {
  window.onload = () => {
    ...
    const pcrButton = document.querySelector(
      ".custom-option-content .pcr-button"
    );
    pcrButton?.style.setProperty(
      "--pcr-color",
      window.templateCustomizer.settings.defaultPrimaryColor
    );
  };
