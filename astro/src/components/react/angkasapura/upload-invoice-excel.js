...
export function useUploadInvoiceExcelLogic() {
  ...
  useEffect(() => {
    ...
    loadCurrentStatus().catch((error) => {
      if (!isCanceled) {
        const message = resolveErrorMessage(error, 'Gagal memuat status upload invoice.');
        setSseError(`Gagal memuat status upload: ${message}`);
      }
    });
    return () => {
      isCanceled = true;
  ...
  useEffect(() => {
    ...
    async function connect() {
      ...
      try {
        ...
        setIsSseFallbackActive(true);
        setSseError(
          resolveErrorMessage(error, 'Tidak bisa membuka SSE status upload invoice.')
        );
        scheduleReconnect();
      }
    }
  ...
  useEffect(() => {
    ...
    const pollJobStatus = async () => {
      ...
      try {
        ...
        if (!isUnmountedRef.current) {
          const message = resolveErrorMessage(error, 'SSE belum tersambung. Polling status gagal.');
          setSseError(`SSE belum tersambung. Polling status gagal: ${message}`);
        }
      } finally {
        pollingInFlightRef.current = false;
      }
  ...
  const handleUpload = useCallback(
    async (file) => {
      ...
      try {
        ...
        const parsedError = parseUploadError(error, 'Gagal mengunggah file. Silakan coba kembali.');
