...
const getRootComputedStyle = () =>
  getComputedStyleSafe && DOC_EL ? getComputedStyleSafe(DOC_EL) : null;
...
const Helpers = {
  ...
  setColor(color, defaultColor) {
    ...
    const ratio =
      (rootStyles ? parseFloat(rootStyles.getPropertyValue('--bs-min-contrast-ratio').trim()) : 0) *
      100;
  ...
  update() {
    if (
      (this.getLayoutNavbar() &&
        ((!this.isSmallScreen() && this.isLayoutNavbarFull() && this.isFixed()) ||
          this.isNavbarFixed())) ||
  ...
  updateCustomOptionCheck(el) {
    if (el.checked) {
      ...
      if (el.type === 'radio') {
        const customRadioOptionList = [].slice.call(
          el.closest('.row').querySelectorAll('.custom-option')
        );
  ...
  syncCustomOptions: (e) => {
    const currEl = document.querySelector(
      `.template-customizer-themes-options input[value="${e}"]`
    );
    if (currEl) {
      currEl.checked = true;
  ...
  syncCustomOptionsRtl: (e) => {
    const currRtlEl = document.querySelector(
      `.template-customizer-directions-options input[value="${e}"]`
    );
    if (currRtlEl) {
  ...
  initCustomOptionCheck() {
    ...
    const custopOptionList = [].slice.call(
      document.querySelectorAll('.custom-option .form-check-input')
    );
