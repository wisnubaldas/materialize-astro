...
export default function UserManagement() {
  ...
  const loadRoles = useCallback(async () => {
    ...
    try {
      ...
      setRoles(normalizeCollection(data));
    } catch (err) {
      setRoles([]);
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      ...
      showToast({
        type: 'danger',
        title: 'User',
        message: resolveErrorMessage(err, 'Gagal membuat user.'),
      });
    } finally {
    ...
    const result = await Swal.fire({
      ...
      preConfirm: async () => {
        ...
        try {
          ...
          Swal.showValidationMessage(resolveErrorMessage(err, 'Gagal mengubah password.'));
          return undefined;
        }
      },
      allowOutsideClick: () => !Swal.isLoading(),
  ...
  const ensureRolesLoaded = useCallback(async () => {
    ...
    const data = await settingClient.listRoles();
    const resolved = normalizeCollection(data);
    setRoles(resolved);
    return resolved;
  }, [roles]);
