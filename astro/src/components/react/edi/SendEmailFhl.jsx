...
export default function SendEmailFhl({ slug }) {
  ...
  const clickSendMail = async (e) => {
    e.preventDefault();
    const email = await promptEmailAddress('Email Send FHL');
    ...
    if (email) {
      try {
        ...
        const toastMessage = resolveErrorMessage(err, 'Gagal mengirim email FHL.');
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!active) return;
        setError(resolveErrorMessage(err, 'Gagal memuat data FHL'));
      } finally {
        if (active) {
          setLoading(false);
        }
