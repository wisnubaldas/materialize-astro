...
export default function FsuSearch() {
  ...
  const handleSendEmail = async () => {
    ...
    const email = await promptEmailAddress('Email Send FSU');
    ...
    try {
      ...
      const message = resolveErrorMessage(err, 'Gagal mengirim email FSU.');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const message = resolveErrorMessage(err, 'Gagal mengambil data host AWB.');
      showToast({ type: 'danger', title: 'FSU', message });
      setError(message);
      setData([]);
      setExpandedRows({});
